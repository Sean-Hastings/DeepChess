'''
THIS IS COPIED FROM THE RAY GIT REPOSITORY BECAUSE IT IS UNAVAILABLE IN
RAY 0.8.6, WHICH THIS PROJECT USES
'''
...
@ray.remote
def put_queue_callback(queue, func, *args, timeout=None, **kwargs):
    value = func(*args, **kwargs)
    queue.put(value, timeout=timeout)


@ray.remote
def put_queue(generator, init, queue, func=pass_through, timeout=None):
    for value in generator(init):
        if isinstance(value, tuple):
            put_queue_callback.remote(queue, func, *value, timeout=timeout)
        else:
            put_queue_callback.remote(queue, func, value, timeout=timeout)


def get_queue(queue, timeout=None):
    try:
        while True:
